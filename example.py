...
st.title(choice)
st.write(f"Раздел {sections[choice]}")
...
if choice == "Общая характеристика ПО":
    ...
    **Streamlit** — это библиотека для создания интерактивных веб-приложений на Python.
    Она позволяет быстро создавать прототипы, презентации и дашборды для анализа данных.
    """)
    st.write("**Основные преимущества Streamlit:**")
    st.markdown("""
    - Простота использования: для создания приложений достаточно знаний Python.
    - Интерактивность: встроенные виджеты (слайдеры, кнопки, выборы).
    - Поддержка визуализации: интеграция с Matplotlib, Plotly, Altair и другими библиотеками.
    - Быстрое развертывание: приложения легко публиковать в облаке.
    """)
    st.code("""
    import streamlit as st
...
# Раздел 2: Установка
    ...
    """, language="bash")
    st.write("После запуска откроется браузер с вашим приложением по адресу `http://localhost:8501`.")
...
# Раздел 4: Таблицы
    ...
    st.write("**Статическая таблица:**")
    st.table(data)
    st.write("**Интерактивная таблица:**")
    st.dataframe(data)
